...
const Sidebar = ({ user, setOpenChatScreen, openChatScreen }) => {
    ...
    const createChat = async () => {
        ...
        if (
            ...
            axios
                .post(
                    "https://us-central1-local-worx.cloudfunctions.net/verifyIfMobileNumber/lookup-number",
                    {
        ...
        <Container openChatScreen={openChatScreen}>
            <Header>
                ...
                >
                    ...
                    <MenuItem
                        onClick={() => handleClose(`/business/${businessId}`)}
                    >
                        My account
                    </MenuItem>
...
const Container = styled.div`
    ...
    height: 100vh;
    overflow-y: scroll;
    // Sets the sidebar to take up full width of viewport at start
    position: fixed;
    width: 100vw;
    max-width: 100%;
    // pushes sidebar -100% to the left when div is clicked
    // the openChatScreen state also triggers chat scren being pulled in from right
    ...
    @media (min-width: 420px) {
        // resets properties to static normal div flow from larger screens
        min-width: 30rem;
        max-width: 3.5rem;
        flex: 0.45;
        position: static;
